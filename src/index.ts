...
/*
function normalizeIPv4(addr: string) {
  return addr
    .split(".")
    .map((n) => +n)
    .join(".");
...
function normalizeIPv6(addr: string) {
  let j = -1,
    expected = 8;
  const groups = addr
    .split(":")
    .map((val, i) =>
      val === ""
        ? ((j = i), "0")
        : val.length <= 4
        ? parseInt(val, 16).toString(16)
        : val
    );
  const IPv4 = groups[groups.length - 1].split("."),
    hasIPv4 = IPv4.length === 4;
  if (hasIPv4) {
    groups.pop();
    expected -= 2;
  }
  if (groups.length < expected && j !== -1) {
    const add: string[] = new Array(expected - groups.length).fill("0");
    groups.splice(j, 0, ...add); // Expand ::
  }
  return groups.length === expected
    ? hasIPv4
      ? compressIPv6(groups.join(":") + ":") + IPv4.map((n) => +n).join(".")
      : compressIPv6(groups.join(":"))
    : ""; // Error. Wrong # of groups.
}
...
function compressIPv6(addr: string) {
  ...
  return len ? (addr.slice(0, i) || ":") + (addr.slice(i + len) || ":") : addr;
}
*/
